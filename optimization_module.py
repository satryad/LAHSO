...
def update_service_capacities(df, service_id, assigned_volume):
    ...
    for index in service_row.index:
        ...
        updated_capacities = [cap - assigned_volume  for cap, sid in zip(capacities, row_service_ids)]
        df.at[index, 'service_capacities'] = ', '.join(map(str, updated_capacities))
        df.at[index, 'service_capacity'] = min(updated_capacities) if updated_capacities else 0
    ...
    for vehicle in vehicles:
        ...
        vehicle_rows = df[df['service_ids'].str.contains(vehicle_str) & (df['Service_id'] != service_id)]
        for index in vehicle_rows.index:
            row_service_ids = df.at[index, 'service_ids'].split(', ')
            capacities = list(map(int, df.at[index, 'service_capacities'].split(', ')))
            updated_capacities = [cap - assigned_volume if vehicle_str == sid else cap for cap, sid in zip(capacities, row_service_ids)]
            df.at[index, 'service_capacities'] = ', '.join(map(str, updated_capacities))
            df.at[index, 'service_capacity'] = min(updated_capacities) if updated_capacities else 0
...
def optimization_model(demands, services, storage_cost, delay_penalty, penalty_per_unfulfilled_demand, solution_number, time_step):

    m = Model(f"synchromodal_freight_transportation")
    m.setParam('Threads', 0)
    m.setParam('MIPGap', 0.1)
    m.setParam('OutputFlag', 0)
    m.setParam('LogFile', '')
    m.setParam(GRB.Param.PoolSolutions, solution_number)  # Set the number of solutions to store in the solution pool
    m.setParam(GRB.Param.PoolSearchMode, 2)  # Set the search mode to focus on finding multiple solutions
    ...
    m.setObjective(
        sum(
            x[d, s] * demands.loc[d, 'Volume'] * (
                services.loc[s, 'total_cost'] +
                storage_hours[d, s] * storage_cost +
                delay_hours[d, s] * delay_penalty + services.loc[s, 'Loading_cost_at_origin'] + services.loc[s, 'Unloading_cost_at_destination']
            ) for d in demands.index for s in services.index
    ...
    for d in demands.index:
        ...
        m.addConstr(sum(x[d, s] for s in services.index) == y[d], f"DemandFulfillment_{d}")
        for s in services.index:
            service_row = services.loc[s]
            # Only proceed if origin and destination match; otherwise, force x[d, s] to 0
            if (service_row['origin'] != demand_row['Origin'] or
                    service_row['destination'] != demand_row['Destination']):
    ...
    for d in demands.index:
        for s in services.index:
            ...
            m.addConstr(storage_hours[d, s] == max(0, services.loc[s, 'first_service_departure'] - demands.loc[d, 'Release Time']-services.loc[s, 'Loading Time']*demands.loc[d, 'Volume']) * x[d, s], name=f"storage_time_{d}_{s}")
            
            # Constraint to calculate delay time if the service's last arrival is after the demand's due time
            m.addConstr(delay_hours[d, s] == max(0, services.loc[s, 'last_service_arrival'] - demands.loc[d, 'Due Time']) * x[d, s], name=f"delay_time_{d}_{s}")

    # Constraint: Do not exceed service capacities
    for s in services.index:
        m.addConstr(
            sum(x[d, s] * demands.loc[d, 'Volume'] for d in demands.index) <= services.loc[s, 'service_capacity'],
    ...
    for d in demands.index:
        for s in services.index:
            ...
            m.addConstr(
                x[d, s]*(services.loc[s, 'first_service_departure'] - demands.loc[d, 'Release Time']) >= services.loc[s, 'Loading Time']*demands.loc[d, 'Volume']*x[d, s],
    ...
    for d in demands.index:
        for s in services.index:
            ...
            m.addConstr(
                x[d, s] * services.loc[s, 'first_service_departure'] >= x[d, s] * demands.loc[d, 'Release Time'],
    ...
    try:
        ...
        if m.status in [GRB.OPTIMAL, GRB.TIME_LIMIT]:
            ...
            for d in demands.index:
                for s in services.index:
                    if best_solution_vars[d, s] > 0.5:  # If demand d is matched with service s
                        assigned_demand = demands.loc[d, 'Volume']
                        services = update_service_capacities(services, s, assigned_demand)
                        best_solution.append({
                            'Time_Step': time_step,
                            'Service_ID': services.loc[s, 'service_ids'],
                            'Service_Capacity': services.loc[s, 'service_capacity'],
                            'Service_first_departure': services.loc[s, 'first_service_departure'],
                            'Service_last_arrival': services.loc[s, 'last_service_arrival'],
                            ...
                            'Number of transshipments': services.loc[s, 'transshipment_time'],
                            'transportation_cost': best_solution_vars[d, s] * demands.loc[d, 'Volume'] * services.loc[s, 'total_cost'],
                            'real_objective': best_solution_vars[d, s] * demands.loc[d, 'Volume'] * (
                                services.loc[s, 'total_cost'] +
                            ...
                            'Solution_Number': best_k + 1,  # Add the solution number
            ...
            for k in range(min(10, m.SolCount)):
                ...
                for d in demands.index:
                    for s in services.index:
                        if solution_vars[d, s] > 0.5:  # If demand d is matched with service s
                            log_entries.append({
                                'Time_Step': time_step,
                                'Service_ID': services.loc[s, 'service_ids'],
                                'Service_week': services.loc[s,'Week'],
                                ...
                                'delay_hours': delay_hours[d, s].X,
                                'Number of transshipments': services.loc[s, 'transshipment_time'],
                                'transportation_cost': solution_vars[d, s] * demands.loc[d, 'Volume'] * services.loc[s, 'total_cost'],
                                'real_objective': solution_vars[d, s] * demands.loc[d, 'Volume'] * (
                                    services.loc[s, 'total_cost'] +
                                    storage_hours[d, s].X * storage_cost +
                                    delay_hours[d, s].X * delay_penalty
                                ),
...
def run_optimization(demands, services, storage_cost, delay_penalty, penalty_per_unfulfilled_demand):

    services['Service_id'] = services.index
    demands['Announce Time'] = demands.index
    num_hours = len(demands)
    all_log_entries = []
    for time_step in range(0, num_hours + 1):
        ...
        if not current_demands.empty:
            ...
            best_solution, all_solutions, services = optimization_model(current_demands, services, storage_cost, delay_penalty, penalty_per_unfulfilled_demand, 1, 1)
