...
def model_train(config, model_input):
    ...
    print(f"total_cost_plot: {len(total_cost_plot)}")
    print(f"total_reward_plot: {len(total_reward_plot)}")
            ...
            if config.print_output:
                ...
                if (
                    ...
                    print_event(
                        ...
                        {config.q_name}_{current_episode}_eps_v2.pkl",
                    )
                    with open(
                        f"q_table/{config.q_name}_{current_episode}_eps_v2.pkl", "wb"
                    ) as f:
                        pickle.dump(dict(model_input.Q), f)
