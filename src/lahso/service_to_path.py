...
def service_to_path(config, output_postfix = ""):
    ...
    data_for_csv = [
        {
            ...
            "service_capacity": min(
                (service["Capacity"]) for service in path["services"]
            ),
    ...
    for _index, row in df.iterrows():
        ...
        if row["transshipment_time"] == 1:
            ...
            if first_service_arrival is not None and truck_travel_time is not None:
                ...
                new_row["service_capacity"] = get_service_capacity(
                    data, first_service_id
                )
            ...
            if first_service_arrival is not None and truck_travel_time is not None:
                ...
                new_row["storage_cost"] = (
                    new_row["storage_time"] * STORAGE_COST_PER_HOUR
                )
                new_row["transshipment_cost"] = updated_transshipment_cost
                new_row["total_transport_cost"] = (
    ...
    new_df["Transshipment Terminal(s)"] = new_df["path"].apply(
        extract_intermediary_points
    )
