...
def comparison(file1, file2, label1, label2):
    ...
    df_comparison["Total Cost Delta"] = df1["Total Cost"] - df2["Total Cost"]
    df_comparison["Total Storage Cost Delta"] = (
        df1["Total Storage Cost"] - df2["Total Storage Cost"]
    )
    df_comparison["Total Travel Cost Delta"] = (
        df1["Total Travel Cost"] - df2["Total Travel Cost"]
    )
    df_comparison["Total Handling Cost Delta"] = (
        df1["Total Handling Cost"] - df2["Total Handling Cost"]
    )
    df_comparison["Total Delay Penalty Delta"] = (
        df1["Total Delay Penalty"] - df2["Total Delay Penalty"]
    )
