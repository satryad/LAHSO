...
for i in range(len(fixed_schedule)):
    if "Barge" in fixed_schedule[i][0]:
        fixed_schedule[i].append(
            (barge_travel_cost1, barge_travel_cost2, barge_handling_cost)
        )
    else:
        fixed_schedule[i].append(
            (train_travel_cost1, train_travel_cost2, train_handling_cost)
        )

fixed_schedule = [
    [list[0]] + [(list[1], list[2], list[3] * 60)] + list[6:] for list in fixed_schedule
]  # Create tuple for schedule
fixed_schedule_dict = {
    fixed_schedule[i][0]: (fixed_schedule[i][0:]) for i in range(len(fixed_schedule))
}
...
truck_schedule = [
    [list[0]] + [(list[1], list[2], list[3] * 60)] + list[6:] for list in truck_schedule
]  # Create tuple for schedule
truck_schedule_dict = {
    truck_schedule[i][0]: (truck_schedule[i][0:]) for i in range(len(truck_schedule))
}
...
s_disruption_profile = s_disruption_profile[
    [
        "Profile",
        "Impact Type",
        "LB Duration",
...
s_disruption_profile["Location"] = s_disruption_profile["Location"].apply(
    lambda s: s.split(", ")
)
s_disruption_profile = s_disruption_profile.values.tolist()
d_disruption_profile = d_disruption_profile[
    ["Profile", "Impact Type", "LB Time", "UB Time", "LB Volume", "UB Volume", "Lambda"]
]
