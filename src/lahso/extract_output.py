...
def shipment_logs(
    ...
    for shipment in shipment_dict.values():
        ...
        total_cost = (
            shipment.tot_shipment_storage_cost
    ...
    df_service_line = pd.DataFrame(
        actual_carried_shipments.items(),
        columns=["Service Line", "Number of Shipments"],
    )
    late_data_df = pd.DataFrame.from_dict(
        late_data, orient="index", columns=["Late time", "Number of late departure"]
    )
    merged_df = df_service_line.merge(
        late_data_df, left_on="Service Line", right_index=True, how="left"
    )
