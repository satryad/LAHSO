...
from policy_function import make_epsilon_greedy_policy
from extract_output import shipment_logs
...
for simulation in range(number_of_simulation):
    ...
    global_vars.s_disruption_event = {node: env.event() for node in (node_list + mode_list)}
    ...
    try:
        print_event(f"Simulation number: {current_episode + 1} starts")
        env = sim.Environment()
        ...
        # initiate rl agent
        rl_module = ReinforcementLearning(env, shipment_dict, mode_schedule_dict, Q, gamma, alpha, policy)
        ...
        if print_output:
            ...
            print("\nReinforcement learning is triggred: ", global_vars.rl_triggers, " times")
            print("\nTotal late departure: ", global_vars.total_late_departure, " minutes")
            print("Total number of late departure: ", global_vars.nr_late_departure, " times")
            print("Average late departure: ", global_vars.total_late_departure / global_vars.nr_late_departure, " minutes")
            print("\nTOTAL COSTS")
            print("----------------------------------------")
            ...
            print(f"Total delay time: {global_vars.total_shipment_delay // 60:02d} hour(s) {global_vars.total_shipment_delay % 60:02d} minute(s)")
            print("----------------------------------------\n")
            total_shipment = len(request_list)
            total_delivered = len(global_vars.delivered_shipments)
            ...
            if training:
                ...
                with open(f'{q_table_path}', 'wb') as f:
                    ...
                    print(f"Total reward per episode is exported as {q_name}")
                if current_episode % 1000 == 0:
                    print_event(f"Q-table is saved as {q_table_path}_{current_episode}_eps")
                    with open(f'{q_table_path}_{current_episode}_eps', 'wb') as f:
                        pickle.dump(dict(Q), f)
            if extract_shipment_output:
                shipment_logs(shipment_dict, global_vars.actual_itinerary, global_vars.actual_carried_shipments,
                              global_vars.shipment_to_rl, global_vars.wait_actions, global_vars.reassign_actions,
                              global_vars.late_dict, current_episode, sd, policy_name, number_of_simulation)
