...
from policy_function import get_q_value
...
class Mode:
    def __init__(self, env, name, schedule, capacity, speed, distance, costs):
        ...
        self.loading_time_window = loading_time_window  # Adjustable parameter
        self.loading = 0
        self.unloading = 0
        self.used_capacity = 0
        self.arrival = {node: self.env.event() for node in
                        node_list}  # Events to signal when the barge/train arrives at a terminal
        self.handling_events = self.env.event()  # Event to signal when the barge finishes loading/unloading
    ...
    def operate(self):
        ...
        while True:
            ...
            self.arrival[self.origin] = self.env.event()  # reset the event in origin location for next depature
            ...
            for shipment in sorted_shipments:
                if shipment[0] not in global_vars.rl_assignment:  # Prioritize undisrupted shipments
            ...
            if self.current_location in global_vars.disruption_location: # if the terminal is disrupted
                print_event(
                    f'{time_format(self.env.now)} - {self.name} departure will be delayed due to disruption at {self.current_location}')
                yield global_vars.s_disruption_event[self.current_location]
            print_event(f'{time_format(self.env.now)} - {self.name} finished loading {self.loading} TEUs at {self.origin}')
            if self.name in global_vars.disruption_location: # if the mode is disrupted
                yield global_vars.s_disruption_event[self.name]
            ...
            if self.env.now < self.departure_time:
                ...
                if self.current_location in global_vars.disruption_location:
                ...
                if self.actual_departure > self.departure_time:
                    ...
                    global_vars.total_late_departure += late_departure
                    global_vars.nr_late_departure += 1
                    if 'Truck' in self.name:
                        name = identify_truck_line(self.name)
                    else:
                        name = self.name
                    global_vars.late_logs.append([name, late_departure])
                    global_vars.late_dict[name][0] += late_departure
                    global_vars.late_dict[name][1] += 1
            ...
            global_vars.possible_paths.loc[global_vars.possible_paths['first_service'] == self.name, 'first_service_departure'] += 168
            global_vars.possible_paths.loc[global_vars.possible_paths['first_service'] == self.name, 'last_service_arrival'] += 168

            # Travel to destination
            if self.name in global_vars.disruption_location:  # Disruption check
                yield global_vars.s_disruption_event[self.current_location]
            yield self.env.timeout(int(self.distance / self.speed * 60))

            # Signal the change of location for arrival
            self.current_location = self.destination
            if self.current_location in global_vars.disruption_location:  # Disruption check
                print_event(
            ...
            print_event(f'{time_format(self.env.now)} - {self.name} arrived at {self.destination}')

            # Simulate container unloading time
            yield self.env.timeout(1)  # Wait for the used capacity to be updated
            yield self.env.timeout(self.handling_time * self.unloading)  # Simulate unloading time

            # For observation
            if 'Truck' in self.name:
                name = identify_truck_line(self.name)
                global_vars.actual_carried_shipments[name] = self.unloading
            else:
                global_vars.actual_carried_shipments[self.name] += self.unloading
            if self.unloading > 0:
                print_event(
            ...
            self.handling_events.succeed()
            self.arrival[self.destination] = self.env.event()  # reset the event in destination location for next leg
            self.status = "Available"
            if 'Truck' in self.name:
                self.truck_service = self.env.event()  # finish the truck service
                self.departure_time = 99999  # reset the truck's departure time
                global_vars.truck_name_list.remove(self.name)
            else:
...
class Shipment:
    def __init__(self, env, request_details):
        ...
        self.state_event = {mode: self.env.event() for mode in (mode_list)}
        self.action_event = {mode: self.env.event() for mode in (mode_list)}
    ...
    def handled(self):
        ...
        global_vars.announced_requests.append(self.name)
        global_vars.unassigned_requests.append(
            [self.name, self.origin, self.destination, self.release_time, self.due_time, self.num_containers,
             self.mode])
        global_vars.active_requests.append(self.name)
        ...
        while self.status != "Assigned":
            try:
                ...
                if self.status == "New Release Time":
                    print_event(f"{time_format(self.env.now)} - {self.name} has a new release time ({self.release_time})")
        ...
        for req in global_vars.unassigned_requests:
            if req[0] == self.name:
                global_vars.unassigned_requests.remove(req)
            ...
            # Check demand disruption
                ...
                if self.status == "New Release Time":
                    ...
                    if self.release_time > self.mode[0].departure_time:
                        ...
                        print_event(f"{time_format(self.env.now)} - {self.name} will miss the service")
                        ...
                        global_vars.requests_to_replan.append(
                            [self.name, self.origin, self.destination, self.release_time, self.due_time,
                             self.num_containers, self.mode])
                        global_vars.disruption_location.append(self.name)
                        self.matching_module.replanning()  # Replan the shipment
                        global_vars.disruption_location.remove(self.name)
        ...
        global_vars.announced_requests.remove(self.name)
        ...
        while self.current_location != self.destination:
            ...
            if 'Truck' in self.mode[0].name:
                ...
                self.mode[0].departure_time = self.release_time + truck_waiting_time
            print_event(
            ...
            while self.status == "Waiting for arrival":
                try:
                    ...
                    self.mode[0].assigned_shipments.append([self.name, self.release_time, self.num_containers])
                ...
                # In case of the shipment is reassigned while waiting
                    ...
                    if 'Truck' in self.mode[0].name:
                        self.mode[0].truck_service.succeed()  # trigger the truck service
                        self.mode[0].departure_time = self.env.now + truck_waiting_time

            # Simulate loading containers onto the mode
            print_event(f"{time_format(self.env.now)} - {self.name} starts loading on {self.mode[0].name}")
            finish_loading = self.env.now

            # Calculate the storage cost
            storage_time = max(0, self.env.now - self.release_time)  # Calculate the storage time
            shipment_storage_cost = (storage_time / 60) * storage_cost * self.num_containers  # Calculate the storage cost
            global_vars.total_storage_time += storage_time  # Calculate the total storage time for all shipments
            self.tot_shipment_storage_cost += shipment_storage_cost
            global_vars.total_storage_cost += shipment_storage_cost  # Calculate the total storage cost for all shipments
            ...
            global_vars.total_handling_cost += loading_cost
            ...
            global_vars.total_storage_time += extra_storage_time
            self.tot_shipment_storage_cost += extra_storage_cost
            global_vars.total_storage_cost += extra_storage_cost
            if self.assigned_to_rl:
            ...
            travel_cost1 = self.mode[0].travel_cost1 * (
                        self.env.now - self.mode[0].actual_departure) / 60 * self.num_containers
            travel_cost2 = self.mode[0].travel_cost2 * self.mode[0].distance * self.num_containers
            ...
            global_vars.total_handling_cost += unloading_cost
            ...
            if self.assigned_to_rl:
                if self.mode[0] != self.mode[-1]:  # Check if the shipment is on the last mode
                    self.rl_start_time = self.env.now  # Update start time for next reward calculation
                    ...
                    self.reward_event = self.env.event()  # reset the reward event for next action
            ...
            if self.name in global_vars.rl_assignment and not self.assigned_to_rl:
            ...
            print_event(f"{time_format(self.env.now)} - {self.name} is available at {self.current_location}")
            global_vars.actual_itinerary[self.name].append(self.mode[0].name)  # for observation
            self.mode.pop(0)  # Remove the completed service from the itinerary

        # Shipment has arrived at the end destination
        self.status = "Delivered"
        print_event(f"{time_format(self.env.now)} - {self.name} has been delivered to {self.destination}")
        global_vars.delivered_shipments.append(self.name)
        global_vars.active_requests.remove(self.name)
        ...
        if self.env.now > self.due_time:
            ...
            global_vars.total_shipment_delay += delay
            self.tot_shipment_delay_penalty += shipment_delay_penalty
            global_vars.total_delay_penalty += shipment_delay_penalty
            ...
            if self.assigned_to_rl:
                ...
                self.reward_event.succeed()
                yield self.env.timeout(1)
                global_vars.rl_assignment.remove(self.name)
        else:
            if self.assigned_to_rl:
                self.reward_event.succeed()
                global_vars.rl_assignment.remove(self.name)
        ...
        global_vars.total_cost += self.tot_shipment_storage_cost + self.tot_shipment_handling_cost + self.tot_shipment_travel_cost + self.tot_shipment_delay_penalty
        self.tot_shipment_reward += self.reward
        global_vars.storage_time_list.append(self.tot_shipment_storage_cost)
    ...
    while True:
        ...
        for act_r in global_vars.active_requests:
            ...
            if shipment[act_r].current_location in locations:
                locations.remove(shipment[act_r].current_location)  # Remove the current location
            if shipment[act_r].current_location in mode_list:
                locations.remove(shipment[act_r].mode[
                                     0].destination)  # Remove the current location destination if a shipment is on a service line
        ...
        for af_r in affected_requests_list:
            ...
            global_vars.requests_to_replan.append(
                [s.name, s.origin, s.destination, s.release_time, s.due_time, s.num_containers, s.mode])
...
class MatchingModule():
    ...
    def planning(self):
        ...
        while True:
            ...
            print_event(f"{time_format(self.env.now)} - disruption at {disrupted_location}")
            request_list = global_vars.unassigned_requests
    ...
    def replanning(self):
        ...
        disrupted_location = global_vars.disruption_location[-1]
        print_event(f"{time_format(self.env.now)} - Replanning due to disruption at {disrupted_location}")
        request_list = global_vars.requests_to_replan
        if disrupted_location in node_list or disrupted_location in mode_list:  # To skip this process for disruption in request
            for request in request_list:
                ...
                self.shipment[request[0]].planning = self.env.event()  # Reset the planning signal
        else:
            self.shipment[request_list[0][0]].planning = self.env.event()  # Reset the planning signal
        ...
        for req in request_list:
            ...
            if not available_next_solution:
                ...
                for path in available_next_solution:
                    ...
                    path_capacity = min(path_capacity) # Determine path capacity from possible itineraries
        ...
        if disrupted_location in node_list or disrupted_location in mode_list or disrupted_location in global_vars.truck_name_list:
            # Start RL assginment
            global_vars.rl_triggers += 1
            rl_match = {}

            # Determine input for RL agent
            for request in request_list:
                if request[0] not in global_vars.shipment_to_rl:
                    global_vars.shipment_to_rl.append(request[0])
                ...
                d_profile = global_vars.d_profile_list[-1][0]
                ...
                action_set_taken = self.rl_module.action_generator(request, current_state,
                                                                   action_sets)  # select the best action

                if request[0] not in global_vars.rl_assignment:  # For first disruption in a request
                    # Setup first assignment to RL
                    global_vars.reward_generator[request[0]] = []
                    if self.shipment[request[0]].status == "Waiting for arrival":
                        ...
                        self.shipment[request[0]].rl_start_time = max(self.env.now, self.shipment[request[0]].release_time)
                        self.shipment[request[0]].assigned_to_rl = True

                else:
                    # Update reward from previous actions
                    if self.shipment[request[0]].status == "Waiting for arrival":
                        storage_time_rl = max(0, self.env.now - self.shipment[request[0]].rl_start_time)
                        self.shipment[request[0]].reward += ((storage_time_rl / 60) * storage_cost * self.shipment[
                            request[0]].num_containers) * -1

                    # Interrupt the previous reward generation process for shipment with multiple disruptions
                    for process_ID in global_vars.reward_generator[request[0]]:
                ...
                for i in range(len(action_set_taken)):
                    ...
                    if i == 0:
                        ...
                        state = current_state
                        future = True
                    global_vars.rg_order += 1
                    if 'Truck' in action_set_taken[i]:
                        action_set_taken[i] = identify_truck_line(action_set_taken[i])
                    reward_gen = self.env.process(
                        self.rl_module.reward_generator(request, state, action_set_taken[i], future, global_vars.rg_order))
                    global_vars.reward_generator[request[0]].append([reward_gen, global_vars.rg_order])

                global_vars.rl_assignment.append(request[0])
                rl_match[request[0]] = (action_sets[0], action_set_taken)
            self.ModeAssignment(request_list, rl_match)
        else:
            self.ModeAssignment(request_list, matching)
        global_vars.requests_to_replan = []  # reset the requests to replan
            ...
            if location != 0:
                if location in mode_list:  # For disruption in service line
                    ...
                    available_paths = available_paths[
                        ~available_paths['Transshipment Terminal(s)'].str.contains(location)]
        return available_paths

    def OptimizationModule(self, request_list, available_paths):

        global_vars.om_triggers += 1
        print_event(f'This is triggers number: {global_vars.om_triggers}')

        # Convert input to df
        df_request_list = pd.DataFrame(request_list,
                                       columns=['Demand_ID', 'Origin', 'Destination', 'Release Time', 'Due Time',
                                                'Volume', 'Mode'])
        ...
        for service in fixed_list:
            ...
            capacitated_service.loc[capacitated_service['service_ids'].str.contains(service), 'service_capacity'] = \
            capacitated_service.loc[capacitated_service['service_ids'].str.contains(service), 'service_capacity'].apply(
                lambda x: max(0, (min(x, free_capacity))))
        ...
        filtered_paths = capacitated_service[
            capacitated_service[['origin', 'destination']].apply(tuple, axis=1).isin(map(tuple, unique_pairs))]
        filtered_paths['Week'] = 1  #Error handling

        # Run optimization algorithm
        matching = om.run_optimization(df_request_list, filtered_paths, storage_cost, delay_penalty,
        ...
        for key, values in matching_result.items():
            ...
            if values == 0:
                new_mode = []
                print_event(f"{time_format(self.env.now)} - {key} has no possible new mode assignment")
            else:
                new_mode = [values]
        ...
        for request in request_list:
            ...
            if new_mode:
                ...
                if old_mode == new_mode:
                    print_event(f"{time_format(self.env.now)} - {request[0]} from {request[1]} to {request[2]} will wait")
                    assigned_mode = []
                else:
                    print_event(
                ...
                for mode in new_mode:
                    if 'Truck' in mode:
                        if len(mode) < 8:
                            global_vars.truck_id += 1
                            name, schedule, capacity, speed, distance, costs = truck_schedule_dict[mode]
                            name = f'{mode}.{global_vars.truck_id}'
                            # Create an object for truck
                            self.mode_schedule[name] = Mode(self.env, name, schedule, capacity, speed, distance, costs)
                            global_vars.truck_name_list.append(name)
...
class ReinforcementLearning():
    ...
    def __init__(self, env, shipment, mode_schedule, q_table, discount_factor, alpha, policy):
        self.env = env
        self.shipment = shipment
        self.mode_schedule = mode_schedule
    ...
    def action_generator(self, request, state, action_sets):
        ...
        vary_seed = np.random.default_rng(int(time.time() * 1000))  # independent random seed for epsilon greedy
        action_id = vary_seed.choice(np.arange(len(action_probs)), p=action_probs)
        chosen_set = action_sets[action_id]

        if action_id == 0:
            print_event(f'{time_format(self.env.now)} - RL choose wait for {request[0]}')
            global_vars.wait_actions[request[0]] += 1
        else:
            print_event(f'{time_format(self.env.now)} - RL choose reassign for {request[0]}')
            global_vars.reassign_actions[request[0]] += 1
    ...
    def reward_generator(self, request, state, action, future, gen_order):
        ...
        try:
            ...
            d_profile = global_vars.d_profile_list[-1][0] if global_vars.d_profile_list else 'no disruption'
            current_time = self.env.now % (1440 * 7) // 60
            next_state = current_location, destination, due_time, volume, d_profile, current_time

        # Get reward
        reward = self.shipment[request[0]].reward
        global_vars.total_reward += reward
        self.shipment[request[0]].reward = 0
        print_event(f'{time_format(self.env.now)} - {request[0]} got reward: {reward} for action: {action}')
        ...
        print_event(f'{time_format(self.env.now)} - {request[0]} take next action: {next_action}')
        ...
        updated_state_vector = current_location_vector, destination_vector, updated_state[2], updated_state[
            3], profile_vector, updated_state[5]
        ...
        next_state_vector = current_location_vector, destination_vector, next_state[2], next_state[3], profile_vector, \
        next_state[5]
        ...
        for reward_gen in global_vars.reward_generator[request[0]]:
            if reward_gen[1] == gen_order:
                global_vars.reward_generator[request[0]].remove(reward_gen)
    ...
    def update_q_table(self, Q, discount_factor, alpha):
        ...
        print_event(f'{time_format(self.env.now)} - Q[s,a] before update: {Q[state][action]}')
        td_delta = td_target - get_q_value(Q, state, action)
        Q[state][action] += alpha * td_delta
        print_event(f'{time_format(self.env.now)} - Q[s,a] after update: {Q[state][action]}')
        self.queue.pop(0)
...
class ServiceDisruption():
    ...
    def generate_s_disruption(self, profile):
        ...
        while True:
            ...
            if IAT != 0:
                ...
                while location in global_vars.disruption_location:  # avoiding the same location
                    if loc_candidate == 'Terminal':
                        ...
                        for mode in mode_candidate_ref:
                            operating = self.mode_schedule[mode].departure_time - start_operation
                            depart = self.mode_schedule[mode].departure_time
                            if 'Truck' in mode:
                                mode_candidate.append(mode)
                            # Localize to operating time window
                            elif operating <= self.env.now <= depart:
                                mode_candidate.append(mode)
                        if mode_candidate:
                            location = np.random.choice(mode_candidate)
                        else:
                            location = 0  # if no service line as candidate to be disrupted
                            break
                        if 'Truck' in location:
                            if global_vars.truck_name_list:
                                location = np.random.choice(global_vars.truck_name_list)
                    count += 1
                ...
                if location != 0:
                    ...
                    global_vars.disruption_location.append(location_new)  # Add location to a list for ongoing disruptions
                    if type == 'Capacity reduction':
                        self.mode_schedule[location].capacity = math.ceil(
                            (1 - capacity_reduction) * self.mode_schedule[location].capacity)
                    global_vars.d_profile_list.append((name, location))  # list for identifying profile for RL state
                    self.env.process(self.execute_disruption(location, duration, name, type, original_capacity))
                    yield self.env.timeout(1)
            else:
                yield self.env.timeout(1)
    ...
    def execute_disruption(self, location, duration, name, type, original_capacity):
        ...
        global_vars.s_disruption_event[location] = self.env.event()
        print_event(f"{time_format(self.env.now)} - Service disruption type {type} at/on {location} starts")
        global_vars.s_disruption_triggers += 1
        if type == 'Capacity reduction':
            print_event(
                f"{time_format(self.env.now)} - {self.mode_schedule[location].name} capacity is reduced to {self.mode_schedule[location].capacity}")
            location_new = location
            global_vars.s_disruption_event[location_new].succeed()
        ...
        if type == 'Capacity reduction':
            ...
            location_new = location
            global_vars.s_disruption_event[location_new].succeed()
        print_event(f"{time_format(self.env.now)} - Service disruption type {type} at/on {location} ends")
        self.disruption_sequence.remove(location)
        location_new = location

        # Update disruption list
        global_vars.disruption_location.remove(location_new)
        global_vars.d_profile_list.remove((name, location_new))
...
class DemandDisruption():
    ...
    def generate_d_disruption(self, profile):
        # global global_vars.d_profile_list
        ...
        while True:
            ...
            disruption = type
            if global_vars.announced_requests:
                disrupted_shipment = np.random.choice(global_vars.announced_requests)
                global_vars.d_profile_list.append((name, disrupted_shipment))
                print_event(f"{time_format(self.env.now)} - Disrupting {disrupted_shipment} with {disruption}")
                if self.shipment[disrupted_shipment].status == "Announced" or self.shipment[disrupted_shipment].status == "Assigned":
                    if disruption == 'Release Time':
                        ...
                        self.shipment[disrupted_shipment].num_containers = math.ceil(
                            self.shipment[disrupted_shipment].num_containers * volume_multiplier)
                        self.shipment[disrupted_shipment].status = "New Volume"
                        self.shipment[disrupted_shipment].process.interrupt()
                yield self.env.timeout(1)

                # Update disruption list
                global_vars.d_profile_list.remove((name, disrupted_shipment))
    ...
    for key, value in shipment_dict.items():
        if value.status != "Delivered":
            ...
            if value.status == 'On board':
                travel_cost1 = value.mode[0].travel_cost1 * (
                            env.now - value.mode[0].actual_departure) / 60 * value.num_containers
